...
export default function ShapChart({ shapSummary }) {
  ...
  const longestLabel = data.reduce((max, d) => Math.max(max, (d.feature || '').length), 0);
  const yAxisWidth   = Math.min(Y_AXIS_MAX, Math.max(Y_AXIS_MIN, longestLabel * CHAR_WIDTH + 12));
  const chartHeight  = Math.max(120, data.length * BAR_ROW_HEIGHT + 20);
  const chartWidth   = containerWidth ? Math.max(CHART_MIN_W, containerWidth) : null;

  return (
    <div
      ref={containerRef}
      className="rounded-xl border-2 p-6 card-shadow"
    ...
    >
      <div className="flex items-center justify-between mb-1">
        ...
        {proxyCount > 0 && (
          <span className="text-xs font-bold px-2.5 py-1 rounded-md"
            style={{ background: '#FFF4E6', color: 'var(--color-ambiguous)' }}>
      ...
      <div className="flex gap-4 mb-4 flex-wrap">
        {[
          ['var(--color-ink)',       'Regular Feature'],
          ['var(--color-ambiguous)', 'Proxy Feature'],
          ['var(--color-biased)',    'Protected Attribute'],
        ].map(([c, l]) => (
        ...
        >
            ...
            tick={({ x, y, payload }) => {
              ...
              return (
                <text
                  ...
                  fill={
                    item?.is_proxy     ? 'var(--color-ambiguous)' :
                    item?.is_protected ? 'var(--color-biased)'    :
                                         'var(--color-ink-mid)'
                  }
                  fontSize={11}
                  fontFamily="var(--font-mono)"
                  fontWeight={item?.is_proxy || item?.is_protected ? 600 : 400}
                >
                  {payload.value}
                </text>
              );
          ...
          <Bar dataKey="mean_abs_shap" radius={[0, 4, 4, 0]} maxBarSize={16}>
            {data.map((entry, i) => (
              <Cell key={i}
                fill={
                  entry.is_proxy     ? 'var(--color-ambiguous)' :
                  entry.is_protected ? 'var(--color-biased)'    :
                                        'var(--color-ink)'
                }
                fillOpacity={0.85}
              />
